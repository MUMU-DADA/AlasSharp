...
| keyevent BACK | `adb shell input keyevent 4` | hit | input keyevent 4 |
| long click (adb form) | `input swipe x y x y 1000~1200`（同点滑动） | hit | input swipe x y x y 1100（上游 long_click 的 adb 形态）→ EQUIPMENT_OPEN match=True score=0.9916；判据取自上游 ship_info_enter |
