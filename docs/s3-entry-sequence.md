...
### 新账号上第一个**跑通**的关卡：9-1（(7,4) → 8×5，40 格）

实测（一条命令，自动导航）：elapsed=**266s**，共 **2** 个战斗步骤，
map_init 行：step=map_init ms=3706.3 error=
=> 9-1 **图可识别且计划跑通**（对比 7-1 同为新账号、却报 No vertical line detected）。
...
| --- | --- | --- |
| 2-1 | (5,3) → 6×4 | ✅ |
| **9-1** | **(7,4) → 8×5** | ✅ **本轮跑通（266s）** |
| 7-1 | (7,2) → 8×3 | ❌ No vertical line detected |
...
**失败样本全部 ≤3 行，成功样本全部 ≥4 行（9 个样本零交叠）** ——
而格数、列数都无法分离（7-1 与 2-1 同为 24 格却一败一成）。

**8-1（(8,2) = 9 列 × 3 行，27 格）实测**（预测失败）：
step=enter_map ms=60196.4 error=GameStuckError: Wait too long | step=enter_map_abort dialog=True red_frac=0.3294 | step=enter_map_retry ms=8454.6 error= | step=map_init ms=5220.7 error=MapDetectionError: No vertical line detected | round=1 step=battle_0 skipped=前一步出错，停止
...
### 1. 进图期间"主动盯防"弹窗（替代 60 秒被动等待）

**用户反馈**："每次舰队没打完地图后再进新图，会有之前地图的撤退提示，我看你每次都没处理，
要等好久才处理" —— 属实：此前是**被动自愈**，要等 `enter_map` 卡满 **60s** 报
`GameStuckError` 才动手 ✗

**改法**：进图期间起后台线程（每 1.5s 一次），检测"正在攻略中"弹窗的像素特征，
一出现就点「撤退」；`enter_map` 返回后停掉线程（`try/finally`）。被动自愈保留作兜底。

**关键取舍**：该线程**刻意走 adb 子进程**（`exec-out screencap` / `input tap`），
**不走引擎设备层** —— 引擎那套是 scrcpy 视频流、**非线程安全**，而本线程与主线程并发 ✗；
adb 每次独立进程，天然安全 ✓（故坐标用固定的 `(479,510)`，弹窗「撤退」，实测有效）。

**验证指标**：跑一张**新图**，`enter_map` 耗时应从 **~68s（60s 超时 + 重试）降到 ~10s** ✓

### 2. 舰队选择打通到命令行

**用户指示**："高难度关卡尽量用舰队 3 4 5" —— 此前舰队被写死为**只用舰队 1**
（那是给旧账号定的默认值 ✗，在日志里可见 `Using fleet: [1, 0, 0]`）。

**改法**：新增 `--fleet1 N --fleet2 N --submarine N`（CLI → `RunCampaignPlan` → 协议 op → `init`），
底层就是 ALAS 的三个配置键 `Fleet_Fleet1 / Fleet_Fleet2 / Submarine_Fleet`（0 = 不用）。

**待确认**：ALAS 只有这三个键，"3 4 5"我按 `--fleet1 3 --fleet2 4 --submarine 5` 理解
（3/4 主攻、5 当潜艇队）；若是别的含义需用户纠正。

### 当前待实测清单（任务结束后的顺序）

1. `dotnet build`（此前 **DLL 被运行中的任务锁定** ✗，编译失败）；
2. 跑一张新图 → 观察 `enter_map` 耗时（验证主动盯防）；
3. 用 `--fleet1 3 --fleet2 4` 跑此前打不过的高章节（如 11-1）验证舰队选择；
4. 汇报后台清图（9-1）的最终结果。


### ✅ 两项新能力实测通过（2026-09-23）

**① 主动盯防弹窗：`enter_map` 68 秒 → 13.2 秒**

```
step=enter_map ms=13195.7 error=（空）
（日志中没有 Wait too long / GameStuckError / enter_map_abort）
```

对比此前（被动自愈）：`enter_map` 要先卡满 **60s** 超时再重试 ≈ **68s** ✗
现在弹窗被后台线程**提前点掉**，主流程从未卡住 ✓ **每进一张图省约 55 秒**。

**② 舰队 3：打过了此前"全军覆没"的 11-1**

```
Using fleet: [3, 0, 0]      ← 舰队选择生效（CLI --fleet1 3）
battle_0 41050 ms ok | battle_6 60700 ms ok
Combat end + GET_SHIP       ← 两战两胜，且有掉落
```
...
### ⚠️ 后台清图作业失败的原因（用户报告"战斗还是卡了"）

```
[批量] 1 关 | [前置] 第 1 关前回战役页 **success=False**
round=1 step=battle_0 skipped（前一步出错，停止）| [结果] elapsed=0.4s
```

**前一次运行把游戏留在"战斗中/未完成出击"里** ✗ → 新作业的"回战役页"导航失败（`success=False`）
→ **0.4 秒就退出**（我的 `campaign` 命令在导航失败时不重试，直接停）。

**两个待改进点**（已识别，待实现）：
1. **导航失败应重试**（而不是立刻放弃）：例如重试 2-3 次、每次间隔几秒；
2. **进图前若处于战斗中/未完成出击，应先处理它**：目前只处理了"正在攻略中"弹窗，
   而"战斗画面本身卡住"需要另一条出路（例如点「撤退」/返回键）—— 用户报告的场景正是这一类。

**当下的处置**：用户已手动把游戏恢复（实测当前 `pages=['page_main','page_main_white']` ✓ 干净）。
