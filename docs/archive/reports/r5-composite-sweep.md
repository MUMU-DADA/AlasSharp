...
- 状态数：**20**（seed=13，每种状态 × 13 原语 × 10 配置）
- 用例数：**2600**；实际比较 **2581**
- 不一致：**0**
- **动作序列对拍**：比较 2581 条，完全相同 2519；干跑前缀（有意偏离）4 条；集合序伪影（只在 `submarine_move_near_boss` 实参上不同）36 条；其余顺序差异 22 条
...
| state8-brute_clear_boss-c8 | primitive_brute_clear_boss | C# [('ensure_fleet', '2'), ('submarine_move_near_boss', 'B1'), ('clear_chosen_enemy', 'B1')] vs 上游 [('submarine_move_near_boss', 'B1'), ('clear_chosen_enemy', 'B1')] |
| state8-brute_clear_boss-c9 | primitive_brute_clear_boss | C# [('ensure_fleet', '2'), ('submarine_move_near_boss', 'B1'), ('clear_chosen_enemy', 'B1')] vs 上游 [('submarine_move_near_boss', 'B1'), ('clear_chosen_enemy', 'B1')] |
