...
| 类别 | 数量 | 含义 |
| --- | --- | --- |
| 精确命中 | 23 | 只在自己的页面上命中（最理想） |
| 共命中 | 7 | 同一屏多条规则同时命中（见下） |
| 从未命中 | 4 | 34 个目标回归画面上一次都没命中 |
...
| `page_dormmenu` | `page_dormmenu`, `page_meowfficer`, `page_private_quarters` |
...
| `page_dormmenu` | `page_meowfficer`, `page_private_quarters` |
...
这些规则在 34 个目标回归画面上一次都没命中 —— 对**受游戏状态阻塞**的页面来说，
