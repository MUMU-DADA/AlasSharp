...
原账号配置已按字节恢复。新 `queue --file` 入口另有 6 tick、6/6 抓帧及往返导航后 4 tick、4/4 抓帧的真机记录，队列/任务/断点/会话交叉审计见 `docs/archive/reports/queue-evidence.md`。
后续 `observe(map=os)` 只读队列在海域 4 次抓帧、4 次检出上游网格，最后为 43 格、零错误；脱敏工件见 `docs/archive/reports/queue-evidence.md`。这些证据不代表主战役地图内正样本、所有截图后端或大世界动作均已验收。
...
迁移前队列入口曾完成主界面到战役页往返及两轮战役页导航；旧逐跳证据见 `docs/archive/reports/queue-evidence.md`，
...
具体路径和边界见 `docs/archive/history/navigation.md`；其他导航路径仍须单独验证。
...
**做完之后**，`docs/archive/history/handover-r0-r2.md` 第十节 C 类里那两条（导航任务断言、诊断后缀断言）一起打勾。
