...
真实产品路径证据：`docs/archive/reports/result-evidence.md` 里 4 条真实通关即由这条链路跑出
...
都没有目标完成或战斗结算。脱敏队列见 `docs/archive/reports/queue-evidence.md`，原件在本地忽略目录
...
脱敏队列、逐任务、断点和会话证据见 `docs/archive/reports/queue-evidence.md`，账号配置已按原始字节恢复；
...
`docs/archive/reports/queue-evidence.md`。本次上游写入的调度时间在探针结束后按运行前配置原字节恢复。
...
`page_main` / `page_main_white`，宿主/设备各初始化一次。新队列入口已分别完成 6 tick 和导航后的 4 tick 真机观测。当前未通关大世界的账号在海域运行 `observe(map=os)` 只读队列，4 次抓帧与 4 次上游地图检测均成功，最后检出 43 格、错误 0；页面命中为空，未执行自律、导航或战斗。原件留在本地忽略目录 `data/mainline-device/20260924-observe-os/`，配置按运行前快照恢复并核对原字节。脱敏交叉审计见 `docs/archive/reports/queue-evidence.md`。
