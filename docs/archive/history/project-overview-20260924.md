...
> 从当前 IR **现算**并产出 [`docs/archive/reports/r3-candidates.md`](../reports/r3-candidates.md)（含逐方法覆盖排序）。
...
`device` / `map`（S2 地图识别，见 `docs/archive/reports/map-detection.md`）/
`map-ir`（关卡 IR 校验）/ `capture`（设备通道对比，见 `docs/archive/history/device-engine.md`）/
...
**S3（上游规则驱动，见 [适配说明](../../s3-upstream-adaptation.md)）**：
...
**设备引擎（多后端可切换，见 `docs/archive/history/device-engine.md`）**：设备 I/O 走宿主，换后端不改 C# 代码。
...
`docs/archive/history/handover-r0-r2.md` 与 `docs/archive/history/session-summary.md`（更早那次设备引擎/S2/S3 的交接，
注意其中的数字是**当时**的快照）。

## 验收记录

**总状态先看 `docs/archive/reports/status.md`**（由 `tools/diagnostics/status.py` 从四份证据文件汇总生成，不手写）。

| 范围 | 状态 | 证据 |
|---|---|---|
| 页面规则（53 个 Page） | **34 真机命中**；8 受游戏状态阻塞；11 原因已定位（其中 9 个岛屿按用户要求跳过） | `docs/archive/reports/page-verification.md` |
| 页面识别全量回归 | **历史 33/34 通过**（退役的直接 `goto` 入口；当前队列入口需重新验） | `docs/archive/reports/regression.md` |
| 识别特异性 | 28 条精确命中 / 5 条共命中（主界面两皮肤、商店三连页）/ 0 条漏检 | `docs/archive/reports/specificity.md` |
| 控件规则（20 模块级 Switch/Scroll + 6 cached_property） | 模块级 **11 命中**、cached 3 命中；其余逐条分类（需更深流程 / UI 版本差异 / 游戏状态阻塞 / 故意不验） | `docs/archive/reports/controls.md` |
| 控制原语（返回键 / 长按 / 滑动 / 文本输入） | 4/4 + 文本输入 3/3（装备码流程，打入内容有截图与像素差双重证据） | `docs/archive/reports/primitives.md`、`docs/archive/reports/text-input.md` |
| 页面导航（队列 `kind=navigate`） | 运行时向上游要图 + 变体择优 + 未建模画面按返回自救；当前队列入口待真机回归 | `docs/archive/history/navigation.md` |
| 结果合同（R0） | `sortie-result/1` 两侧对拍；历史日志 4 条通关 / 2 起清理或导航撤退；结构化工件 1 条本局撤退，0 矛盾 | `docs/result-contract.md`、`docs/archive/reports/result-evidence.md` |

单项文档：

| 文档 | 内容 |
|---|---|
| `docs/archive/reports/page-verification.md` | 53 个页面规则的真机导航验证（**数字以本页与 `docs/archive/reports/status.md` 为准**，二者由脚本从证据文件重建） |
| `docs/archive/history/navigation.md` | 页面导航图（控制能力）：图从上游运行时获取、变体择优、真机多跳运行记录 |
| `docs/archive/reports/controls.md` | 20 个控件规则（Switch/Scroll）+ 滑动控制的真机验证与总账 |
| `docs/result-contract.md` | R0 结果合同：结果词表、不变量、跨关复位定义、怎么被强制执行 |
| `docs/archive/reports/result-evidence.md` | R0 实机结果证据核对（脚本从历史日志与脱敏运行工件重建，不手写） |

## 路线图

当前路线已重排为 [R0-R5 整体迁移路线](../../architecture-roadmap.md)：先冻结结果真值和证据，再建设常驻运行时，按业务域做通用垂直切片，之后迁移原生钩子，最后做前端和宿主替换评估。S0/S1/S2/S3 是已验收基线，不代表已有独立 C# 关卡引擎。

| 阶段 | 交付目标 | 状态 |
|---|---|---|
| R0 真值与证据 | 统一成功/撤退/错误/限额结果合同，清理历史证据矛盾 | 合同与要求的真机证据门槛已验证，本局撤退已纳入审计 |
| R1 常驻运行时 | 设备/宿主会话、取消超时、结构化日志、诊断工件、统一错误 | `observe`/`navigate` 仅经队列入口执行；观测核心任务有历史只读真机证据，当前队列入口待真机回归，见 [`docs/runtime.md`](../../runtime.md) |
| R2 任务域垂直切片 | 战役批量 → 大世界 → 活动 → 周期任务，按域复用通用状态模型 | 十类业务任务及导航/观测已实现；大世界/活动完整动作和其余周期执行路径仍待完成，见 [`docs/tasks.md`](../../tasks.md) |
...
不把撤退算作通关。详见 [`docs/result-contract.md`](../../result-contract.md)、
[`docs/archive/reports/result-evidence.md`](../reports/result-evidence.md)。
...
2. **S3 引擎实现**按调用频次从高到低做，前 5 个方法覆盖绝大多数关卡：
   ...
   钩子清单见 `alashub verify` 的「引擎钩子」段（49 个关卡、去重 17 个方法；以 docs/archive/reports/r3-candidates.md 为准）。
