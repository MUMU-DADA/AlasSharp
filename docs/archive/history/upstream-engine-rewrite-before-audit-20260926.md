...
> 以下为审查前的阶段记录，不是当前验收结论。原有“0 阻塞”“导出全绿”“全量可调用”及只改本地状态等说法已发现证据不足或实现错误，不能作为迁移放行依据。
> 当前边界见[引擎重写说明](../../upstream-engine-rewrite.md)，当前状态见[迁移路线](../../architecture-roadmap.md)。此归档仅调整相对链接并修复两处文字控制字符；原始运行日志、截图、工件及哈希未改动。
...
决策层与寻路都有**全库对拍**证据；**原语的真机动作层对照、`loop` 域接线与域级切换尚未完成**。
...
| 钩子可执行率（**钩子**口径） | 2951 个 `battle_*` 钩子 | **计划语言扩展后**：`plan_complete=false` 从 163 降到 **118**（新增"观察步骤 + 局部绑定 + 分支体"，见下）；这些钩子由引擎**拒绝执行并报原因**，不静默跳过 | `verify_r5_coverage.py` / `r5_incomplete_hooks.py` |
| 原语覆盖 | **34** 个已登记原语 | 32 个被夹具执行过（覆盖率改由执行器**实际调用记录**统计，不再靠静态扫描夹具文本）；`check_accessibility` 已由复合原语扫描对拍、但暂无夹具钩子路径（原因写在 `ALLOWED_GAPS`） | `verify_r5_coverage.py` |
...
| 目标选择（过滤 DSL） | 200 个随机状态 × 2166 例 | 与上游 Filter **不一致 0**（真实过滤器串 + 两个优先级预设 + preserve）。中途曾报 421 处不一致，查明是**对拍用的上游替身漏了 `encode()` 的 boss/land 分支**（把 boss 当成 1E）造成的假红——挡在检查套件里之后这类漂移不会再溜过去（`verify_r5_selection_sweep.py`，上限 0） | [选择扫描](../../archive/reports/r5-selection-sweep.md) |
...
#### 与上游的**已知差异**登记（不是"待办清单"，是已定位、有证据的偏离）

| 差异 | 上游行为 | C# 行为 | 为什么这样定 | 影响 |
| --- | --- | --- | --- | --- |
| 寻路的收敛程度 | `find_path_initial` 用"前沿不再增长就停"，`visited`/`grid_connection` 都是 `set`（身份哈希序）→ 收敛到哪个不动点取决于迭代序，还会出现"格子的 cost 比它自己的 connection 链还贵" | **relax 到不动点**（`CampaignPathfinder` 类注释写明） | 上游那套顺序不可复现；定点收敛让 C# 的 cost 场与自己的 connection 链自洽，且**不可能比上游更贵**（更贵判为硬失败） | 无伏击配置逐格完全相同；有伏击配置 52 处「上游偏大」（1370 关 / 275,934 格） |
| 干跑中的设备相关循环 | `fleet_2_protect` 最多 20 轮、`clear_all_mystery` 的 `while 1`，都靠设备状态变化才停 | 干跑不改变状态，只做一轮就退出（两个方法里都写明） | 离线干跑不能假装设备状态变了 | 对拍里单列为"干跑前缀"（4 条）；`clear_all_mystery` 因此**不做**对拍 |
| 计划不完整的钩子 | 这些钩子在上游**有真实语句**（嵌套 `if` 等静态无法表达的部分），导出时 `plan_complete=false` | 引擎**拒绝执行**并报 `unparsed` 里的原因（tier C 守卫，与 `BattlePlanRunner` 同口径）；`r5-exec` 有夹具用例断言这一点 | 宁可停下报原因，也不能把"什么都没做"当成钩子执行完毕 | 163/2951 个钩子受此影响（`verify_r5_coverage` 每次报出该数） |
| `ClearCaughtBySirenFlags` | 逐格改 `GridInfo.is_caught_by_siren`，**无设备动作** | 两个宿主都在自己的模型上做同样的事，设备宿主**不调用渠道** | 原先误判为"未移植" | `r5-device` 自检：残留标记 0、设备调用 0 |

> **一次误判的记录（留在这里防止重犯）**：路障子集选择一度被登记为上表的差异，理由是"C# 定点收敛会找到更小的子集"。
> 做决定性验证时发现**根本不成立**：诊断命令 `r5-select` 的 C# 侧没给 `primitive_brute_clear_boss` 接 kind，
> 于是静默落进"选一个敌人"的默认分支——对拍比的是**默认选择器**而不是 `brute_clear_boss`。
> 接上分派后 80 例全部一致。两处修复：① 补齐 `primitive_brute_clear_boss` / `brute_fleet_meet` /
> `clear_potential_boss` 三种 kind；② **未知 `primitive_*` kind 现在直接报错**，不再用默认分支兜底。
...
1. **能力归属矩阵**：逐 `IVisionEngine` 方法 + Core 调用点（**已成文件**：[能力归属矩阵](../../archive/reports/capability-ownership.md)，由 `r5_capability_matrix.py` 重建），标注【识图保留 / 可静态化 / 必须自研 / 设备帧】；
...
#### 一次授权运行的执行清单（照抄即可，2026-09-26 核对）

前置（**只读**，已实测通过）：`adb devices` 能看到 `127.0.0.1:16384	device`；游戏停在**章节页**、
没有正在进行的出击。任何一项不满足就**不要**开始（中止条件见下）。
...
- **中止条件**：`r5-diff` 报出任何**决策层**差异；运行中出现阻塞原因；页面不在预期位置（识别帧可查）。
  任一命中就停下报告，不做任何重试或兜底。
...
**真机日志 × 真机帧**（硬模式 1-4：`data/s3_native_1_4.log` + `data/fixtures/map_hard_1_4.png`，
**两者不是同一局**，帧比日志早一天）：

| 侧 | 结果 |
| --- | --- |
| 决策层 | `battle_0`×3 → `battle_3`，**4/4 一致** |
| 动作层（帧驱动） | 上游打 `C1/C2/F3/G3`，C# 打 `C2/G3` → **目标有交集（C2、G3）** |
| 动作层（**不给**识别） | C# 一个格子都没打（声明地图上没有敌人）→ **没有交集** |

最后一行是关键对照：**结论来自"识别 → 状态"这条路，不是巧合**。C# 没打到 `C1`/`F3` 的可能原因也
如实记着：两局不是同一场（帧早一天），且干跑没有舰队所在格（成本场退化）——这些都写在"状态来源"行里。
...
无目标不一致 + 包装层原语被如实列出；运行目录用例核对自解析；**真机口径用例**（上面那张表的后两行：
帧驱动必须有交集、不给识别必须没有交集）在日志与帧都存在时才跑，缺文件跳过并说明。已登记进 `verify_all.py`。

#### P2-25 路线层对照（"走的是不是同一条路"）

在决策层与动作层之外加第三层：**上游走位序列 vs C# 走位序列**。

| 侧 | 来源 |
| --- | --- |
| 上游 | 日志里的 `[Fleet_1: C1]` 位置标记（上游 `logger.attr_align` 打的），按出现顺序去掉连续重复 |
| C# | **动作目标的首次出现顺序**——`clear_chosen_enemy(X)` 在上游本身就包含 `goto(X)`，所以它的目标就是"走到了哪" |

输出：两串走位 + 共同格子 + **相对顺序是否一致**（不要求逐格相同：干跑状态不刷新，C# 会在同一格反复出手）。
**路线层是参考项，不影响退出码**——它是给人看的证据，不是判据。
...
顺带修掉一处口径噪声：`[Fleet_1: …]` 这类**纯日志标记**不是引擎原语，已从原语对照里排除，
否则它们会以"只有上游用到"的形式污染结论（路线层已经消费了这些信息）。

**对拍**：`verify_r5_diff.py` 的真机口径用例增加断言——路线层必须有共同格子、相对顺序一致、
纯日志标记不出现在 `only_upstream`。
