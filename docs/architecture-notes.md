...
本文保留此前调查的依赖关系与历史决策，行号和规模可能已变化。当前目标已明确为 C# 执行全部非视觉逻辑、Python 仅保留必要 CV/OCR；下文“先保持现路线、以后再评估替换”的阶段结论已被[最新引擎评估](upstream-engine-rewrite.md)取代。当前状态以[迁移路线](architecture-roadmap.md)为准。
