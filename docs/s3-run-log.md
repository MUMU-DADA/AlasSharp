...
### round 143：1-4 第二次失败的真实原因 —— **欠检**（不是过检）

对第二次失败存下的帧（`_map_init_fail_campaign_1_4_att1.png`，01:26:30）离线复算：

| 帧 | 检出 | `inner_h` 线 | `inner_v` 线 |
| --- | --- | --- | --- |
| 第二次失败帧 | **无检出**（`shape=None`） | **2** | 1 |
| 好帧 | ✓ `[6,2]`＝21 格 | 4 | 1 |

⇒ 该机位下**内部水平线只有 2 条**（好帧 4 条），不足以建网格 → 检测失败 →
`map_init` 里的 `ensure_edge_insight` 反复滑动找边界 → 触发
`GameTooManyClickError: MAP_SWIPE_0_2`。**与"行数过检"无关**（那个假设证伪）。

含义：`brute` 垫片去掉的是一个**虚假的硬失败**（消失点退化），1-4 因此变成"能跑通但不稳定"；
剩下的不稳定性来自**某些机位下采样到的线太少**（真实困难），而当前的恢复策略是"不停滑动"，
会撞上点击上限。

**下轮修法（小而局部）**：把恢复策略从"无限滑动"改成**有限次 + 换手段** ——
`ensure_edge_insight` 只试一次，之后要么换机位重试，要么**撤出重进**（重进会重新摆相机），
并捕 `GameTooManyClickError` 当"该机位不可用"处理，而不是让它把整轮带崩。
