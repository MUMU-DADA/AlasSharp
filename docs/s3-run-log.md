...
> - 新号：**未探**（换号瞬间游戏停在标题页、未登录）。恢复登录后要重做三件事：
>   ① 探哪些章/关可进（`s3_preflight` + 逐章 `ensure_chapter`）；
>   ② 看各关进度（`stage_progress.py`，找出**还没三星**的图 —— 那才是能拿到
>      `威胁排除 %`/星级**变化**的验收目标）；
>   ③ 确认舰队槽位（旧号的 3/6 规则对新号是否成立；`Fleet_Fleet1/2` 是配置项）。
>
