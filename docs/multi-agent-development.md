...
- 任务沿用上述目标、基线、范围、验收和交付字段。回执含边界、分支、提交、完成项、验证证据、阻塞和下一步；`status.json` 只保留当前快照，至少含 `updated_at`、`state`、`last_inbox`、`branch`、`commit`。`commit` 必须是实际 Git 哈希，状态为 `working`、`blocked`、`ready_for_review` 或 `done`；历史发现留在消息中，不累加到状态文件。
- 第三方 agent 开工前、每个阶段完成或阻塞时检查信箱并更新状态；长任务至少每 5 分钟一次。主 agent 约每 20 分钟监收，在阶段完成或失败时复查，优先推进自身任务；重复未变的状态无需打扰用户。未收到回执不能宣称对方已开始。
- 检查信箱须按序读取 `last_inbox` 之后的新消息，先回执最新任务再开工；收到纠偏后不得继续把旧问题列作待答复。每个完整功能切片集中提交和回报，不把单行微调拆成多轮交付，也不在验收已通过后无限扩充无关测试。
- 主 agent 用一份当前审阅总账记录各切片的验收结论、负责人和主线提交。分支自测通过与主线已集成分开标记；收到替代任务回执后可归档已处置消息，保留引用索引和原始证据。
