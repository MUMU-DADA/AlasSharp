...
`map.select` `clear_chosen_enemy` `check_accessibility` `goto` `mob_move` `fleet_at` `device.disable_stuck_detection` `__getattribute__` `fleet_2_rescue` `air_strike` `bored_visit` `fleet_boss.clear_chosen_enemy` `battle_boss` `fleet_2.clear_chosen_mystery` `fleet_ensure` `fleet_1.clear_chosen_enemy` `execute_actions` `_goto` `clear_potential_boss` `fleet_boss.battle_default` `siren_list.pop` `device.sleep`
