...
`map.select` `clear_chosen_enemy` `check_accessibility` `goto` `mob_move` `fleet_at` `device.disable_stuck_detection` `__getattribute__` `fleet_2_rescue` `air_strike` `fleet_boss.clear_chosen_enemy` `fleet_2.clear_chosen_mystery` `fleet_ensure` `bored_visit` `battle_boss` `siren_list.pop` `fleet_boss.battle_default` `fleet_1.clear_chosen_enemy` `_goto` `clear_potential_boss` `device.sleep` `execute_actions`
