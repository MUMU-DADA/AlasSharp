...
`map.select` `clear_chosen_enemy` `check_accessibility` `goto` `mob_move` `fleet_at` `device.disable_stuck_detection` `__getattribute__` `fleet_2_rescue` `air_strike` `fleet_2.clear_chosen_mystery` `bored_visit` `battle_boss` `fleet_boss.clear_chosen_enemy` `fleet_ensure` `fleet_boss.battle_default` `clear_potential_boss` `execute_actions` `siren_list.pop` `_goto` `device.sleep` `fleet_1.clear_chosen_enemy`
