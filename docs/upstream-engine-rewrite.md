...
| 目标选择（过滤 DSL） | 200 个随机状态 × 2166 例 | 与上游 Filter **不一致 0**（真实过滤器串 + 两个优先级预设 + preserve）。中途曾报 421 处不一致，查明是**对拍用的上游替身漏了 ncode() 的 boss/land 分支**（把 boss 当成 1E）造成的假红——挡在检查套件里之后这类漂移不会再溜过去（erify_r5_selection_sweep.py，上限 0） | [选择扫描](archive/reports/r5-selection-sweep.md) |
