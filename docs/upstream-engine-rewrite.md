...
| 复合原语 | 20 个随机状态 × **6 原语** × 8 配置 / 944 例 | 与上游**真实方法**（`clear_enemy`/`clear_any_enemy`/`clear_siren`/`clear_boss`/`clear_roadblocks`/`clear_potential_roadblocks`）**不一致 0**；2 处集合序伪影、16 处上游自身崩溃如实跳过 | [复合扫描](archive/reports/r5-composite-sweep.md) |
