...
| 路障**子集**选择（`brute_clear_boss`） | `brute_find_roadblocks` 枚举敌人子集，返回**第一个**让目标可达的子集；上游的提前终止可能给出较大的子集 | 定点收敛下可能返回**更小**的等价子集 → 排序后打的那一格可能与上游不同 | 与上一条同源；两边都满足"清一个路障就能到"，但不是同一格 | **暂不纳入对拍**（`KNOWN_DIVERGENCE`），登记在此；已定位、待决定性验证后再决定是否对齐 |
