...
## 检测失败与正常负样本（2026-09-24）

`op_map_detect` 只有在上游 `View.load()` 抛出 `MapDetectionError` 时返回
`load=negative`；没有网格、检测线或相机位置不可用都沿用该上游负样本语义。
其他加载异常返回 `load=error`，构造异常保留在 `construct_error`，预测异常保留在
