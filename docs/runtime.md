...
dry-run 同样遵守关卡边界取消：尚未读取的关卡保存跳过工件，批次与任务记为 `cancelled`，
不能因跳过项没有出击结果而误报上游失败；取消前已发生的读取或合同错误仍保留为失败。
...
任务开始的 `boundary_state` 记录最后观测状态：设备会话取原生设备缓存，离线会话取宿主帧，
`frame_source` 标明来源；无缓存记为不可用。它不额外抓帧，也不作为通关判据或实时画面保证。
...
快照原子替换遇到 Windows 短暂占用时有界重试同一临时文件；持续占用或其他 IO 错误明确失败，
保留旧快照和原始日志。调度收尾落盘失败记入返回结果，仍移除日志处理器并关闭文件，
不把旧状态快照或调度器正常返回当作工件完整。`verify_native_scheduler.py` 用真实 Windows 文件句柄
验证短暂/持续占用，并覆盖日志收尾失败传播及资源释放；不操作游戏。
...
```powershell
Alas.Server report --run <运行目录> --json <报告.json>
Alas.Server runs --artifacts <工件根目录> --limit 10
Alas.Server --port 8765
...
本地控制台使用 .NET 10 Kestrel，仅监听 `127.0.0.1`，忽略环境中的 URL/端点覆盖配置。由
`Alas.Server` 启动；服务参数 `--root`、`--repo`、`--data`、
...
队列 API 接收普通队列 JSON，提供 dry-run、只读与显式动作授权三种模式，通过 `QueueExecution.RunFile` 执行。
共享 UI 提供实例配置、任务及调度启停；任意队列 JSON 可通过 API 或 Server 的 `queue` 命令提交。
运行中展示已写入的任务工件，结束后生成完整报告；停止使用同一边界停止原语。
草稿与工件默认位于 `.runtime/control/`，可用 `--workspace`、`--artifacts` 调整。
程序目录含 `ui/index.html` 时自动同源托管 Avalonia WASM，也可用 `--ui-root <预构建 wwwroot>` 指定；没有 Web UI 时仍提供 API。
静态目录必须有 `index.html`，拒绝目录浏览、路径穿越和链接逃逸，只读取有限 MIME 类型；服务仍只监听
回环地址，静态托管不等于远程认证或 HTTPS。业务依赖（`tools`、`data`、上游仓库和 Python 环境）
仍需由发布布局显式提供。
`tools/publish_server.ps1` 使用项目内 SDK 和 NuGet 缓存发布指定 RID 的服务器，默认复制已生成的
`.runtime/ui-publish/browser/wwwroot` 到 `ui/`；仅需 API 时可指定 `-ApiOnly`。它不打包 Python、上游仓库或设备后端，启动时仍需显式传入这些路径。
...
宿主分发前安装 `deploy_storage.py` 通用存储垫片，保留上游读配置、重定向与自动保存子类；写入只合并本对象实际改变的字段，保留新模板字段，使用与 Core 相同的 OS 文件锁，同字段过期写入明确失败。原始上游与第三方上游均通过合成文件回归，Windows 跨 Python/.NET 锁已实测；其他平台的锁互操作尚待验收。
此接口当前证明配置持久化和宿主写入兼容，尚未完成启动运行消费、部署器迁移、远程服务、认证和更新/重启接线；项目之外单独运行未安装垫片的旧部署器不在此保护范围内。
...
| Python/Core 部署存储协同 | `verify_deploy_storage.py`（两个上游原始类、字段保留、重定向、冲突与跨语言锁） |
