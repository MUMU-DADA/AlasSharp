...
1. **汇总事实**：队列/批次结论、逐任务原请求（`required` / 完整 `input`）与逐关卡条目、日志计数（条目/错误/警告）、
   宿主与设备初始化次数、工件数量；
...
后续 `observe(map=os)` 只读队列在海域 4 次抓帧、4 次检出上游网格，最后为 43 格、零错误；脱敏工件见 `docs/queue-evidence.md`。这些证据不代表主战役地图内正样本、所有截图后端或大世界动作均已验收。
...
| 内容 | 在哪 | 报告里 |
| --- | --- | --- |
| 任务原请求（`id` / `kind` / `required` / `input`） | `queue.json` 每条任务摘要、`task-*.json` | **内联**；HTML 任务表可展开完整输入 |
...
`queue.json` 的任务摘要保留原请求，离线回喂 `queue --file` 时不会丢失域输入或
`required`。旧运行的摘要若未记录这两个字段，报告仅从对应的 `task-*.json`
补读；该工件也缺失时字段保持缺席，HTML 显示“未记录”，不将缺失解释成默认输入。
...
**它为什么长这样**：静态报告采用单文件形式，便于归档，也用于检查数据面是否足够完整。
该选择只约束报告产物；统一桌面与远程网页的新要求已经触发 R4 重新选型，见
[R4 统一 UI 技术调研](r4-ui-architecture.md)。
...
**静态报告的边界**：该归档产物不引入前端框架，继续由生成期排好结构。
交互控制台按新的 R4 选型推进；两种界面都消费同一运行时事实，不另判结果。
