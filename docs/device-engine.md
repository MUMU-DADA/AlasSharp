...
## ⚠️ 常驻 runner 暴露的真问题：`map_detect` 在非地图画面上会**间歇性误报**

`alashub run --map main` 在 `page_campaign_menu` 上跑，统计出 **命中 5/9 tick、网格数=14**；
用 Python 循环做同样的"抓帧 → map_detect"对照，**2/8 命中** —— 不是 C# 计数器的锅，
而是**识别本身在这个画面上会偶尔报出网格**。

为什么此前没发现：S2 的"负样本保证"只用过**一张静止 fixture**（`os_map.png`）验证过一次，
那张确实被正确拒绝；但实机上的 `page_campaign_menu` 是**战役章节选择页**，画面里有章节预览图
（本身含海图/岛屿/网格状纹理），于是偶发误报。**单帧 fixture 的负样本验证不够**。

同一轮还发现另一条上游错误路径（在非地图画面上反复调用后出现）：
`AttributeError: 'Points' object has no attribute 'x'`（t03 起连续出现，疑似上游与新 numpy 的
又一处不兼容，或该分支假定了一定形状的返回值）。

### 建议的加固（下一步）

1. **时序一致性闸门**：连续 2 帧都判定为"在地图上"且网格形状一致，才认为真的在地图上
   —— 误报通常是单帧偶发，导航/战斗循环本来就按 tick 迭代，这个约束几乎不增加成本；
2. **负样本扩样**：把 `page_campaign_menu` 等**易混画面**抓成 fixture 入库，纳入
   `verify_map_detection.py` 的负样本集合（现在的负样本只有 1 张）；
3. 查 `Points object has no attribute 'x'`：定位是上游哪条分支（可能是 numpy 2 的又一处不兼容）。

### 已修复：**战场判据「地图上必定有船」**（误报 6/10 → 0/10）

实测对照（`data/fixtures/menu_*.png` = 战役章节选择页 10 帧，`map_*.png` = 四张真地图）：

| 画面 | detected_raw | grids | shape | 逐格标志 | **船标志** |
| --- | --- | --- | --- | --- | --- |
| menu_02/04/07/08（误报帧） | True | 14 | [6,1] | **0** | **0** |
| 真地图 2-1 | True | 24 | [5,3] | 4 | **3** |
| 真地图 10-4 | True | 48 | [8,5] | 5 | **5** |
| 真地图 困难 1-4 | True | 21 | [6,2] | 4 | **4** |
| 真地图 活动图 | True | 30 | [7,3] | 3 | **3** |

**判据**：`detected = 检出网格 AND 至少一个船标志`（is_enemy/is_boss/is_siren/is_fleet/
is_current_fleet/is_submarine）。道理直白 —— **在战斗中画面上不可能没有己方舰队**。
`detected_raw` 保留原判便于诊断；可用 `require_ships=false` 关闭。

**实测效果**：10 帧易混画面 **误报 6/10 → 0/10**；四张真地图**全部仍然检出**（ships 3/5/4/3）。

> 附带否决的方案：**时序一致性闸门救不了这个场景** —— 误报是**连续帧稳定**的
> （静态预览图逐帧一致），所以"连续 2 帧一致"照样通过。判据必须来自**画面内容语义**（有没有船），
> 而不是时间维度。

#### 船判据只对战役模式生效（否则会误杀整类海域图）

第一版把判据无差别套用，产品路径立刻从 **5/5 掉到 4/5** —— 因为**海域图（mode=os）的逐格标志
在本客户端本来就为空**（OS 网格类的模板与本客户端图标不匹配，S2 阶段已查明并记录），
于是整类海域图被判成"非战场"。
