...
新执行图已经直接编译上游素材与页面声明，C# 管理识别阈值、动画命中选择、按钮偏移、计时和导航；纯视觉进程只返回图像测量。原生四服声明、路径、识别轨迹及 630 项素材图像对照通过，具体范围见[迁移路线](architecture-roadmap.md)。恢复处理与完整游戏任务仍有缺口，新诊断入口仅支持只读观测；本轮没有连接设备，不新增实机结算证据，也不改变旧 S3 生产路径或冻结结果合同。
