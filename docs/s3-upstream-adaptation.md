...
本页的原生调用链是尚未替换域的过渡实现。最新目标为全部非视觉逻辑由 C# 重写，Python 仅保留必要的 CV/OCR；“完整消费上游规则”要求保留语义，不要求永久执行 Python。迁移时逐域以等价 C# 产品路径接替，禁止逐图/逐页旁路与伪造结算。目标边界、实际归属和门槛见[引擎重写](upstream-engine-rewrite.md)。
