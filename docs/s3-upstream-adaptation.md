...
本页的原生调用链描述旧产品现状。最新目标为全部非视觉逻辑由 C# 重写，Python 仅保留必要的 CV/OCR；“完整消费上游规则”要求保留语义，不要求永久执行 Python。新引擎整体独立建设并切换产品组合入口，不把模块逐个嵌回旧流程，不提供 Python 业务回退；仍禁止逐图/逐页旁路与伪造结算。目标边界、实际归属和门槛见[引擎重写](upstream-engine-rewrite.md)。

规则文件中的配置、MAP 和 Campaign 钩子也在迁移范围：静态声明使用 C# 类型化定义或纯数据资源，可执行钩子直接成为 C# 方法，不再经过 JSON 步骤与计划解释器。完整保留上游已有的地图差异允许且必要，不能在公共引擎另造地图特例。本页记录的导出与原生运行合同在对应消费者替换前继续生效，现有通关证据仍只证明原生路径。

新执行图已经直接编译上游素材与页面声明，C# 管理识别阈值、动画命中选择、按钮偏移、计时、导航与通用弹窗/剧情恢复；纯视觉进程只返回图像测量。原生四服声明、路径、识别轨迹、630 项素材图像对照及 716 个恢复场景通过，具体范围见[迁移路线](architecture-roadmap.md)。独立诊断入口支持只读观察及 C# 导航，完整游戏任务仍有缺口；本轮没有连接设备，合成 ADB 重放不新增实机结算证据，也不改变旧 S3 生产路径或冻结结果合同。

独立 C# 会话相机已接入原生地图 UI 干扰优先级、阶段入口确认、初始化/刷新几何错误确认及越界纠正；四服入口与信息条通过上游图像对照。恢复仅由可执行相机调用，`map_observe` 仍只读。此为新引擎离线验收，不更改旧产品 `CampaignRun.load_campaign()` 或历史成功结算证据；OS 相机、完整出击及新 C# 结算仍未完成。

新地图权威状态由 `Alas.Engine/Runtime/CellState` 忠实实现 `GridInfo.merge/decode/wipe_out/reset`，保留观测优先级、部分拒绝前已发生的状态写入、机关联动及每局隔离。地图声明与观测分开；`ME/Me` 不产生额外寻敌优先级，清格也不表示整场战役通关。原生状态对照的范围与命令见迁移路线；旧生产的地图扫描、原生调度和既有结果证据未改写。

## 生产调用链

以下仍为旧产品的运行链。独立 `Alas.Engine` 的地图检测现已直接迁入 C#：类型化章节配置、线段筛选/拟合、单应性校准、中心/角点/轮廓分支及视野几何，与纯 CV 数值桥和 C# ADB 组合。通用根因修复包括 Hough 运算精度、灰度模板重复舍入、主地图/大世界 mask 使用位置及首次相机边缘校正，没有新增逐地图规则。原生对照、历史截图回放、队列/CLI 只读入口和失败证据通过，计数及命令见[迁移路线](architecture-roadmap.md)。这证明对应检测与离线调用语义，不证明真实移动、地图弹窗恢复或 C# 战役成功结算；旧 S3 生产链、冻结结果合同和既有成功证据未改写。
...
原生运行的可选影子观测限定于本次 `Campaign.run()`、当前线程和钩子选择日志，
通过响应中的 `shadow_observation` 传回 C#；不再按修改时间寻找并读取共享日日志。
旧实现会混入其他出击、覆盖同关重复尝试的影子工件，并可能因日日志被占用而中断批次索引写入。
现在影子工件跟随逐关工件的唯一名称，观测缺失、格式错误或落盘失败均告警并保留原始出击结论；
空观测和有跳过的观测不算一致，dry-run 不产生影子结论。变体从原生运行配置读取。
`verify_r5_observation.py` 覆盖当前运行隔离、被锁旧日志、重复尝试、故障和缺证据；
`verify_r5_host_contract.py` 使用原生无设备替身核对观测注册与释放。
这只证明已观测轮次的钩子选择，不证明原语动作等价、完整迁移或新增真机通关；
原生加载、入口、战斗调度、冻结结果合同和历史成功结算证据保持原样。
...
- R5 原语循环原先对录制宿主单独执行一轮就返回，神秘格达到 100 次上限仍静默返回假，掩盖未完成状态并放行后续调用。现统一从宿主当前地图重选，舰队保护保留原生 20 轮；神秘格在无目标时返回假，耗尽实验预算仍有目标则阻塞。`verify_r5_primitive_loops.py` 用原生 Map/CampaignMap 和两侧相同的逐动作状态帧验证 15 个结束、敌人靠近/消失、边界轮数场景，另验 3 个预算拒绝场景；先复现 4 个错误场景再修复通过。固定录制状态不代表真实状态演进，预算拒绝不是上游通关条件，生产原生流程与历史成功证据保持原样。
- 旧复合扫描用自建 Fleet 代理、丢弃返回值和参数，并豁免动作前缀/顺序/同权目标，不能证明完整等价。严格扫描现直接运行原生 Map/Fleet 方法及 C# 注册原语，核对完整调用和业务状态；默认 4,080 例中 681 例差异或未验证，报告与退出码保持失败。原生异常、未完成寻路或集合顺序差异均需继续定位，不将这些样本全部归咎于引擎，也不以抽样对照替代产品接线和真机结果。
...
普通模式 5-3 合并 5-1 与自身的七项章节 Config，由上游处理谜题清理、BOSS 刷新后相机恢复和战斗调度。
五场战斗后取得 S 级成功结算，`sortie-result/1` 为 `cleared` 且 0 违例；
同队列后续实时抓帧确认 `page_campaign`、`in_map=false`。账号配置 SHA-256 前后相同，
脱敏归档 `tools/diagnostics/evidence/20260925T213335/` 经结果审计通过；未验关卡仍保留缺口。

普通模式 5-4 合并 5-1 与自身的九项章节 Config，由上游处理道路阻塞、潜在阻塞、BOSS 可达性及相机恢复。
五场战斗后取得 S 级成功结算，`sortie-result/1` 为 `cleared` 且 0 违例；
同队列后续实时抓帧确认 `page_campaign`、`in_map=false`。账号配置 SHA-256 前后相同，
脱敏归档 `tools/diagnostics/evidence/20260925T214108/` 经结果审计通过。同轮船坞容量处理只有执行痕迹，
不据此独立断言退役结果；未验关卡仍保留缺口。

## 引擎复刻审查与宿主边界

独立 `Alas.Engine` 的地图初始化与全图扫描已按上游公共调用链迁入 C#：清理模式覆盖先于循环声明装载，累计出生表按战斗次数取行，保留堡垒/弹跳缺失数、动态遮挡、必扫机位、冲突恢复及舰队清格的 BOSS 被抓例外。未增加地图名、页面或阈值补丁，也未将新模块接回旧计划解释器。732 个原生初始化/缺失预测对照与 22 项合成相机检查通过；这些仅证明离线语义和取消/超时边界，真实相机、战役动作及 C# 成功结算仍未验证。既有原生成功工件和合同不变，阶段缺口统一见[迁移路线](architecture-roadmap.md)。

扫描链现补上 C# 视野几何、滑动预测和相机实现，直接从多边形与图像数值构造观测，不引入运行时 JSON 计划或 Python 业务转发。修复的通用问题包括透视消元精度导致边界格坐标偏移，以及完整格子预测中的潜艇抑制不能代替滑动用原始舰队标记。保留上游过滤/归一化、投票并列拒绝、边缘优先级、零格待确认状态、聚焦/预设/反向恢复和滑动避让规则；287 个视野、420 个位移、288 个状态转换、24 条动作轨迹、3 组避让/倍率、5 条等待稳定轨迹及 32 组 CV 图像配对通过。输入与末端设备均为合成测试，真实检测/mask/手势和地图中断恢复尚缺；本轮没有真实出击或新增成功结算，旧结算合同与证据不变。

对 `31fb4d2` 起的复刻改动进行审查后，生产战役仍由 `CampaignRun.load_campaign()`、章节 Config 合并和原生 `Campaign.run()` 完成；C# 执行器与设备宿主仅作离线对照和替换评估。导出摘要不能作为运行时配置源，已有成功结算证据不变，本轮未新增真机通关样本。

宿主协议曾把原生 False/None 字符串化、吞掉调用错误、用缺省计数或旧地图继续决策，以及漏写弹药、被抓标志和拾取列表。现保留标量类型、错误与类型化控制流，写入先获原生确认，动作后刷新地图；缺失必需字段明确失败。潜在 BOSS 使用原生 `fleet_boss` 属性路径，拾取列表通过原生列表读取和追加。授权校验在中间属性解析前进行，防止带动作的 property 绕过门禁。这些是通用接缝修复，不增加地图或页面特例。

离线宿主合同使用原生地图对象与无设备替身覆盖错误、状态写入、元组参数、控制流和拒绝反例；双宿主对照使用同一显式计数反馈，比较完整动作、舰队路径、目标、参数和逐轮状态。固定地图、离线 Ended、撤退及影子一致均不代表通关。严格导出与调用审计继续保留源缺失、不完整计划和无法证明的继承绑定失败，当前未完成项见[迁移路线](architecture-roadmap.md)。
