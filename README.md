...
| 程序 | 位置 | 说明 |
| --- | --- | --- |
| `alashub.exe` | `src/Alas.DataTool/bin/<配置>/net10.0/` | CLI 与本地控制台 |
| `Alas.Server.exe` | `src/Alas.Server/bin/<配置>/net10.0/` | 独立服务端 |
...
`.runtime/publish/<程序名>/`，`-SelfContained` 再额外带上 win-x64 运行时（每个约 108 MB），
可以拷到没装 .NET 的机器上运行。
