...
**截图字节路径与文件路径的判定结果不一致**（实测 40 例中 11 例）。

- 文件路径：`screenshot_load` → 上游 `load_image`（PIL）
- 字节路径：`screenshot_set` → 上游 `screenshot_adb` 的解码序列（`cv2.imdecode` + `BGR2RGB`）
- **真实设备走的是字节路径**，所以这个差异必须在接入 adb 之前查清。
- 已排除的原因：灰度素材（PIL mode `L`）在本检查里已单独跳过并计数——`imdecode(IMREAD_COLOR)`
  会强制三通道而 PIL 保留二维，这是**两条路径的固有差异而非 bug**（且真机截图恒为三通道，
  字节路径更贴近生产）。
- 剩下 11 例的原因**尚未定位**。在查清之前，该项**不计入验收判据**，
  既不假装通过，也不让它污染"验收通过"的结论。
