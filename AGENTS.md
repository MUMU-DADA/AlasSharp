...
- 现有导出与计划解释链按消费者逐步退役；离线展示、来源追踪、漂移检查和对照仍有独立价值。构建期辅助生成若有需要，应直接产出可编译、可审阅的 C# 源码，不把中间计划带回产品运行时。
- 现有 `CampaignRun.load_campaign()` / `Campaign.run()` 等仅是尚未替换域的过渡兼容与离线真值来源。替换须有独立对照、真实产品路径和结果证据；不能把兼容路径通过算作 C# 通过，也不能在 C# 模式失败后静默执行 Python 业务。
