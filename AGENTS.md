...
- 忠实迁移上游已有的每张地图、页面和服务器规则允许且必要；禁止的是在适配层另造坐标、阈值、路线或流程特例。规则须记录上游来源和版本，保留配置覆盖、继承调用顺序及状态语义；仅把文件后缀改成 C# 不算迁移完成。
- 新产品执行图整体移除导出计划、步骤解释和 Python 业务转发；离线展示、来源追踪、漂移检查和对照仍有独立价值。构建期辅助生成若有需要，应直接产出可编译、可审阅的 C# 源码，不把中间计划带回产品运行时。
- 现有 `CampaignRun.load_campaign()` / `Campaign.run()` 等只作为旧产品现状和离线真值来源。新架构须有独立对照、真实产品路径和结果证据；不能把旧路径通过算作 C# 通过，也不能在 C# 模式失败后执行 Python 业务。
- 最终视觉桥只接受图像、模板、识别参数并返回观测，不运行任务、不调度、不点击设备、不维护权威游戏业务状态。CV 库本身也并非只能用 Python；暂保留桥接是复用现有识别能力的实现选择。
- Python 离线导出器、上游参考执行和回归脚本可以保留，但不得成为产品业务执行依赖；修测试不计为完成 C# 迁移。现有静态摘要不自动升级成完整运行时规则。
- 本轮直接在 `master` 开发、审阅和提交，不创建新分支或 worktree；有并行 agent 时先划分不重叠文件责任。此项优先于下文旧的分支隔离流程。

当前事实、迁移顺序和完成门槛见 [上游引擎重写](docs/upstream-engine-rewrite.md) 与 [迁移路线](docs/architecture-roadmap.md)。
