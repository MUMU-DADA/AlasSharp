...
完整路线与理由见 [整体迁移路线与不可变边界](docs/architecture-roadmap.md)。后续工作统一沿“契约链 → 通用运行时 → 宿主边界 → 原生钩子”的顺序推进：先保证上游数据和运行时入口一致，再扩大 C# 的通用能力，最后按覆盖面迁移钩子。

以下约束视为本地协作规范，不因单张地图、单个活动或一次现场结果改变：

- 生产战役必须继续走上游 `CampaignRun.load_campaign()`、章节 `Config` 合并和原生 `Campaign.run()`；JSON 导出仅用于离线展示、溯源和漂移校验。
- 视觉、页面和地图识别继续通过上游对象解析和 `IVisionEngine`；不得从 `assets.json` 重建运行时规则，也不得在 C# 保留第二份素材或地图特例表。
- C# 图像代码只能作为参考实现与对拍基准；替换上游宿主前必须有逐项对拍和产品路径回归证据。
- 任何失败先定位通用调用链、状态清理或客户端兼容根因；禁止按地图名称、编号、尺寸或少数案例增加阈值、坐标、机位、路线、寻敌或战斗分支。
- 通关证据必须是成功结算并返回章节页；撤退、超时、未知退出或单张截图不能替代真实通关。
- 修改这些边界时，提交说明必须写明根因、上游语义、影响范围和新增验证；并通过 `python tools/diagnostics/verify_architecture.py`。
