...
def main():
    ...
    lines += [
        ...
        '（属"相机窗口"情形：地图比可见区宽）。',
        '',
        '检出：mode=main 与 mode=os **都失败**，reason=`Vanish point and distant point too close`'
        '（Perspective.load 的退化几何判断）。',
        '',
        '`map_detect_trace` 数字：',
        '',
        ...
        '消失点几何因此退化。',
        '',
        '关卡 IR：困难图复用同章节地图数据（`data/campaign/campaign_hard` 下只有 '
        'campaign_12_4 / 14_4 / campaign_hard 三个文件），故取普通 `campaign_1_4`：',
        ...
        '所以正确做法是**失败后降阈值重试**的回退策略（与上游自己的 '
        '`search_tile_center → corner → rectangle` 多策略同思路），而不是改默认值。',
        '',
