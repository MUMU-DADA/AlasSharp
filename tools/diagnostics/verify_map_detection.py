...
def main():
    ...
    lines += [
        ...
        '### 下一步（二选一，都能离线复跑）',
        '',
        '1. **调映射**：按上面两档的失败模式做一次小扫描（线的目标亮度 200±20、海 170±20），',
        '   找到能让 `View.load` 成功的组合，然后固化成客户端 profile；',
        '2. **先排除取样时机**：再抓一张"进图停稳 15 秒后"的样本，'
        '若它在上游原版参数下就能检出，则说明这张 fixture 是入场动画期间抓的，映射不必改。',
        '',
        '（2 需要一次授权出击；1 完全离线。）',
        '',
        '## 复现',
