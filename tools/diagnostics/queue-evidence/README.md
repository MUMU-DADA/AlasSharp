...
`20260925T040837` 在原生初始化后的 1-1 地图只读观测 4 秒，章节 Config 透传后 4/4 检出 7 格，执行错误为 0；
准备阶段另记为 stopped_after_map_init/incomplete，未战斗，不算通关。`20260925T040915` 是独立的动作授权清理队列，
由原生导航回主页并实时抓帧确认不在地图内。两份来源均在 `data/mainline-device/live-20260925-observe-*`，
准备和清理不混记为只读动作，原始日志与截图仍留本地忽略目录。
