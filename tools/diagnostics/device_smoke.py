...
CHECKLIST = [
    ...
    ('控件规则 + 控制原语 + 文本输入',
     ...
     '`verify_text_input.py` 会往装备码输入框打字（已跑通 3/3）；原语（返回键/长按/滑动）已跑通 4/4，安全。**三项都已在窗口内跑过**：原语 4/4、文本输入 3/3、控件规则 15 命中/13 未命中/5 被拦（33 项）—— **被拦的 5 项读法（已更正一次，以此为准）**：闸门是"模板分 ≥0.85 才点"，被拦说明**那一刻目标控件没有被确信看见**，是**画面/时机**问题，不是素材问题。我先前写过"本客户端模板与上游存图不一致"，**那是错的**：拿同一台设备的真机地图帧复测，`handler/STRATEGY_OPEN` 模板分 **1.000**（颜色 5.80）、`handler/IN_MAP` 1.000、`ui/BACK_ARROW` 1.000、`map/FLEET_NUM_1` 1.000 —— 模板是匹配的。所以这 5 项的正确状态是**"还没验到"**（要先把画面走到它能被确信看见的状态再跑），**不要去改上游素材、也不要放宽闸门**。'),
