...
CHECKLIST = [
    ...
    ('控件规则 + 控制原语 + 文本输入',
     ...
     '`verify_text_input.py` 会往装备码输入框打字（已跑通 3/3）；原语（返回键/长按/滑动）已跑通 4/4，安全。**三项都已在窗口内跑过**：原语 4/4、文本输入 3/3、控件规则 15 命中/13 未命中/5 被拦（33 项）—— **被拦的 5 项读法**：脚本的"确信才点"闸门用的是**模板分**（≥0.85），而本客户端这些控件的模板与上游存图不一致；以 `handler/STRATEGY_OPEN` 为例，实测模板分 0.330 被拦，但**颜色判据是匹配的**（在真机地图帧上 similarity=5.80 < 阈值 10，且该素材确实带 file）。上游产品路径判这些控件用的是 `appear`（颜色），所以**这不是产品缺陷、不要去改上游素材或产品判定**——它是"验证闸门比产品判定更严"造成的口径差。'),
