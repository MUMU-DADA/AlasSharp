...
New-Item -ItemType Directory -Force -Path (Join-Path $runRoot 'tools'), (Join-Path $runRoot 'data') | Out-Null
Copy-Item -LiteralPath (Join-Path $repo 'tools/control_ui.html') -Destination (Join-Path $runRoot 'tools/control_ui.html')
...
try {
    ...
    $page = Get-Http "$base/"
    if (-not $page -or $page.StatusCode -ne 200 -or $page.Content -notmatch 'Alas') {
        throw 'Detached published server did not serve the copied control page.'
    }
}
finally {
...
Write-Output 'PASS: publish output boundary, link-safe cleanup, environment restoration, offline publication and detached HTTP start.'
