...
internal static class HitTestReachabilityChecks
{
    ...
    private static void CheckButtonsAreHittable(Control view, string page)
    {
        ...
        try
        {
            ...
            if (buttons.Count == 0)
            {
                // 诊断：把该页所有按钮（不论过滤条件）的可见性/启用状态打出来，区分"没渲染"与"被过滤排除"。
                foreach (var any in view.GetVisualDescendants().OfType<Button>())
                {
                    var ancestor = any.GetVisualAncestors().OfType<Control>()
                        .FirstOrDefault(c => !c.IsVisible);
                    Console.WriteLine($"DIAG {page}: button={any.Name ?? any.Content?.ToString() ?? "?"} " +
                        $"visible={any.IsVisible} enabled={any.IsEnabled} effectively={any.IsEffectivelyEnabled} " +
                        $"bounds={any.Bounds} hiddenAncestor={ancestor?.Name ?? "(none)"}");
                }
                var row = view.GetVisualDescendants().OfType<Control>().FirstOrDefault(c => c.Name == "SettingsQueueRow");
                Console.WriteLine($"DIAG {page}: queueRow={(row is null ? "(absent)" : $"visible={row.IsVisible} bounds={row.Bounds}")}");
                Console.WriteLine($"NOTE: {page} renders no clickable buttons in this state");
                return;
            }
    ...
    private static readonly Dictionary<string, string> PrimaryActions = new()
    {
        ...
        ["settings page"] = "SettingsApplyButton",
    ...
    private sealed class StubRemoteBackend : RemoteAccess.IRemoteAccessBackend
    {
        ...
        public Task<RemoteAccess.RemoteAccessStatus> ReadStatusAsync(CancellationToken t = default) =>
            Task.FromResult(new RemoteAccess.RemoteAccessStatus("ready", "https://remote.example/abc"));
    ...
    private sealed class StubSettingsBackend(bool longText = false) : Settings.ISettingsBackend
    {
        ...
        public Task<Settings.SettingsSnapshot> SaveAsync(IReadOnlyList<Settings.SettingsChange> changes, CancellationToken t = default) =>
            Task.FromResult(Snapshot(longText));
    }
