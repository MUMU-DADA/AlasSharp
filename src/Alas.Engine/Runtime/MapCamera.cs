...
public sealed class MapCamera : IMapScanCamera
{
        ...
        TimeProvider? clock = null, Random? random = null)
    {
        ...
        _camera = new(map.Map.Shape, initialPosition, initialView);
    ...
    private async ValueTask<bool> SwipeCoreAsync(ViewCell requested, CancellationToken token)
    {
        ...
        if (_rules.Optimize)
        {
            _observation ??= await _recognition.ObserveAsync(View, Position, token: token);
            (preferred, forbidden) = SwipeAreas(_map, Position, View.Geometry, _observation, vector);
        }
    ...
    private async ValueTask UpdateCoreAsync(bool waitSwipe, CancellationToken token)
    {
        ...
        {
            token.ThrowIfCancellationRequested();
            next = await _source.CaptureAsync(token);
